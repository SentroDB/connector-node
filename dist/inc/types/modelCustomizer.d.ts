...
export type TableNameOf<S> = S extends {
    readonly tableNames: readonly (infer N)[];
...
export type PayloadByOp<T extends DBManagerSchema.TableName, O extends Operation> = O extends "CREATE" ? Record<string, unknown> : O extends "READ" ? GetDataBody : O extends "UPDATE" ? {
    where: Record<string, unknown>;
    patch: Record<string, unknown>;
} : O extends "DELETE" ? Array<RowOf<T>> : never;
export type ResultArrayByOp<T extends DBManagerSchema.TableName, O extends Operation> = O extends "READ" ? ListOf<T> : O extends "CREATE" ? RowOf<T>[] : O extends "UPDATE" ? RowOf<T>[] : O extends "DELETE" ? RowOf<T>[] : never;
...
export type FieldWriter<T extends DBManagerSchema.TableName, K extends string> = (value: unknown, ctx: BaseContext<T, "CREATE" | "UPDATE">) => Promise<Record<string, unknown> | void> | (Record<string, unknown> | void);
